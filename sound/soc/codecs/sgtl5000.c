...
static int sgtl5000_fill_reg_cache(struct snd_soc_codec *codec)
{
	...
	/*printk("--------sgtl5000_fill_reg_cache\n");*/
...
static int sgtl5000_check_id(struct snd_soc_codec *codec)
{
        u16 reg;
	int rev;
...
static int sgtl5000_power_init(struct snd_soc_codec *codec)
{
        u16 tmp;

        tmp = snd_soc_read(codec, SGTL5000_CHIP_LINREG_CTRL);
        tmp |= 0x8;
        snd_soc_write(codec, SGTL5000_CHIP_LINREG_CTRL, tmp);

        tmp = snd_soc_read(codec, SGTL5000_CHIP_ANA_POWER);
        tmp |= SGTL5000_LINEREG_D_POWERUP;
        tmp &= ~SGTL5000_LINREG_SIMPLE_POWERUP;
        snd_soc_write(codec, SGTL5000_CHIP_ANA_POWER, tmp);

        tmp = snd_soc_read(codec, SGTL5000_CHIP_LINREG_CTRL);
        tmp |= SGTL5000_VDDC_ASSN_OVRD;
        tmp |= SGTL5000_VDDC_MAN_ASSN_VDDIO << SGTL5000_VDDC_MAN_ASSN_SHIFT;
        snd_soc_write(codec, SGTL5000_CHIP_LINREG_CTRL, tmp);

        tmp = snd_soc_read(codec, SGTL5000_CHIP_REF_CTRL);
        tmp |= (0x1f << SGTL5000_ANA_GND_SHIFT);
        snd_soc_write(codec, SGTL5000_CHIP_REF_CTRL, tmp);

        tmp = snd_soc_read(codec, SGTL5000_CHIP_LINE_OUT_CTRL);
        tmp |= (0x22 << SGTL5000_LINE_OUT_GND_SHIFT);
        tmp |= (SGTL5000_LINE_OUT_CURRENT_360u << SGTL5000_LINE_OUT_CURRENT_SHIFT);
        snd_soc_write(codec, SGTL5000_CHIP_LINE_OUT_CTRL, tmp);

        tmp = snd_soc_read(codec, SGTL5000_CHIP_REF_CTRL);
        tmp |= SGTL5000_SMALL_POP;
        snd_soc_write(codec, SGTL5000_CHIP_REF_CTRL, tmp);
        ...
        tmp |= SGTL5000_I2S_IN_POWERUP;
        tmp |= SGTL5000_I2S_OUT_POWERUP;
        tmp |= SGTL5000_DAP_POWERUP;
        tmp |= SGTL5000_DAC_EN;
        tmp |= SGTL5000_ADC_EN;
        snd_soc_write(codec, SGTL5000_CHIP_DIG_POWER, tmp);
...
static int sgtl5000_power_up(struct snd_soc_codec *codec)
{
	u16 tmp;
...
static int sgtl5000_probe(struct snd_soc_codec *codec)
{
	...
	ret = sgtl5000_check_id(codec);
	if (ret)
		return ret;

//	sgtl5000_fill_reg_cache(codec);

	ret = sgtl5000_power_init(codec);
	if (ret)
		return ret;
