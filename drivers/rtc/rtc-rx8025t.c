...
static int rx8025_get_time(struct device *dev, struct rtc_time *dt)
{
	...
	dt->tm_mon = bcd2bin(date[RX8025_REG_MONTH] & 0x1f);
...
static int rx8025_set_time(struct device *dev, struct rtc_time *dt)
{
	...
	date[RX8025_REG_MONTH] = bin2bcd(dt->tm_mon);
...
LOG();
	//rx8025->rtc = rtc_device_register(&client->dev, client->name,
...
static const struct of_device_id __maybe_unused rx8025_dt_ids[] = {
        { .compatible = "epson,rx8025",        .data = 0, },
        { }
};
